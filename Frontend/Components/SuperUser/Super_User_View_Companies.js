...
import CompanyPDF from '../Spoc/CompanyPDF';
const secretKey = "Kartik's Secret Key";

const Super_User_View_Companies = ({route}) => {
  ...
  const handleViewDetails = async company => {
    try {
      ...
      if (cachedCompanyData[companyId] === undefined) {
        let i = '0';
        let t = {email: i, cycle_id: cycle.cycle_id, user_id: company.user_id};
        let response = await fetchAPI(
  ...
  const handleInfLinkPress = company => {
    const jsonData = decryptStringToJSON(company.inf);
  ...
  const handleJafLinkPress = company => {
    const jsonData = decryptStringToJSON(company.jaf);
              ...
              <>
                <View style={styles.hrContainer}>
                  ...
                  <Text style={styles.hrInfo}>
                    ...
                    {company.inf === null ||
                    decryptStringToJSON(company.inf) == null ? (
                      'Not Available'
                    ) : (
                      <TouchableOpacity
                        style={{marginTop: 18}}
                  ...
                  <Text style={styles.hrInfo}>
                    ...
                    {company.jaf == null ||
                    decryptStringToJSON(company.jaf) == null ? (
                      'Not Available'
                    ) : (
                      <TouchableOpacity
                        style={{marginTop: 18}}
              ...
              expandedCompaniesDetails.includes(company.company_id) && (
                <>
                  {Object.keys(decryptStringToJSON(company.company_details))
                    .length > 0 ? (
                    <View style={styles.hrContainer}>
                      <Text style={styles.companyName2}>Company Details</Text>
                      <Text style={styles.hrInfo}>
...
export default Super_User_View_Companies;
